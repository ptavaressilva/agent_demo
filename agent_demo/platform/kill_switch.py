"""Kill switch: an operator-controlled stop for any registered agent, checked
at the start of every invocation -- including resumes of a paused
human-in-the-loop approval -- so an incident can be contained without a
redeploy.
...
`harness.run()` call. `set_killed()` invalidates the cache so the operator
making the change sees it take effect immediately; other already-running
...
There is one kill switch shared by every agent registered in this process --
it is a platform-wide stop, not per-agent, since `harness.run()` calls
`KillSwitch(mongo).check()` unconditionally regardless of which `AgentSpec`
it was given.
"""
...
class AgentKilledError(Exception):
    """Raised by `harness.run()` when the kill switch is engaged. Carries the
...
class KillSwitch:
    # Shared across instances (a new KillSwitch is constructed per
    # `harness.run()` call) but scoped per Mongo client + collection, keyed
    # by `id(client)` since the process-wide client (see
    # `platform.mongo.get_mongo_client`) is stable for the life of the
    # process.
