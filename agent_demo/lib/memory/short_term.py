...
Scope is a single ongoing conversation -- it is *not* shared across sessions
(that's what long-term memory is for). Shared infrastructure any agent may
use; not enforced by the platform the way kill switch/tracing/gateway are.
"""
...
def build_checkpointer(client: MongoClient) -> MongoDBSaver:
    return MongoDBSaver(
        client=client,
        db_name=platform_settings.mongo_db_name,
