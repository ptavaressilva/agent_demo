...
from agent_demo.platform.hitl import is_approved, request_approval

_SCHEMA_PATH = Path(__file__).resolve().parent.parent.parent.parent.parent / "db" / "schema.sql"
...
async def get_pool() -> asyncpg.Pool:
    ...
    if _pool is None:
        _pool = await asyncpg.create_pool(
            dsn=house_search_config.postgres_dsn, min_size=1, max_size=5
        )
...
def build_listing_tools(pool: asyncpg.Pool, session_id: str) -> list[BaseTool]:
    ...
    async def draft_viewing_request(
        ...
        decision = request_approval(
            ...
            "Approve drafting this viewing request for the buyer to review?",
        )
