...
503 mapping. Runs against a fake Mongo client (see
tests/platform/test_kill_switch.py) and a patched `harness.run`, so no real
Mongo/Postgres/MCP/gateway is needed.
"""
...
import main
from agent_demo.platform.kill_switch import AgentKilledError
...
def admin_token(monkeypatch):
    monkeypatch.setattr(main.platform_settings, "kill_switch_admin_token", "s3cret")
    return "s3cret"


def test_admin_endpoint_fails_closed_when_token_not_configured(fake_mongo, monkeypatch):
    monkeypatch.setattr(main.platform_settings, "kill_switch_admin_token", "")
...
async def test_invoke_maps_agent_killed_error_to_503(monkeypatch):
    async def _raise_killed(agent, payload):
        raise AgentKilledError("on fire")

    monkeypatch.setattr(main.harness, "run", _raise_killed)
...
async def test_invoke_routes_admin_action_without_touching_run(fake_mongo, admin_token, monkeypatch):
    async def _boom(agent, payload):
        raise AssertionError("harness.run() should not be called for an admin-action payload")

    monkeypatch.setattr(main.harness, "run", _boom)
...
async def test_invoke_admin_action_payload_never_reaches_invoke_request_validation(fake_mongo, admin_token):
    ...
    `buyer_id` -- proves it's short-circuited before request validation
    rather than falling through to harness.run()."""
    result = await main.invoke({"_kill_switch_admin_action": {"token": admin_token}})
