...
database. Runs through the real platform graph factory (with an in-memory
checkpointer) since `interrupt()`/`Command(resume=...)` need actual LangGraph
persistence -- but against a fake Postgres pool, so no real database is
needed.
"""
...
from agent_demo.agents.house_search.tools.postgres_tools import build_listing_tools
...
async def test_draft_viewing_request_writes_only_after_approval():
    ...
    graph = build_react_graph(
        model, [draft_tool], InMemorySaver(), InMemoryStore(), system_prompt_fn=_system_prompt_fn
    )
